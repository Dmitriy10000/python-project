...
function toggleButtons(activeButton) {
	...
	if (activeButton===1) {
		...
		.then(data => {
			...
			document.getElementById('Section_friends').innerHTML = '';
			...
			data.friends.forEach(friend => {
				...
				friendLogo.classList.add('friend_logo');
				...
				listItem.appendChild(friendLogo);
				var friendName = document.createElement('div');
				friendName.classList.add('friend_name');
				friendName.innerHTML = friend.user_name;
				listItem.appendChild(friendName);
				document.getElementById('friends_list').appendChild(listItem);
			});
		})
	}
}